...
let norm ((x,y,z,t):tuple) : tuple = 
  if t = 1 then failwith "normalizing point" else 
    let m = magn (x,y,z,t) in div (x,y,z,t) m

  let dot ((x1,y1,z1,t1):tuple) ((x2,y2,z2,t2):tuple) : float = 
    if t1 = 1 || t2 = 1 then failwith "dot of point" else
      (x1*.x2)+.(y1*.y2)+.(z1*.z2)
  
  let cross ((x1,y1,z1,t1):tuple) ((x2,y2,z2,t2):tuple) : tuple =
    if t1 = 1 || t2 = 1 then failwith "cross of point" else
